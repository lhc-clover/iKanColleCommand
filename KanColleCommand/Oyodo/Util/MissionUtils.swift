...
func getMissionData(byId: Int) -> MissionData? {
    ...
    case 210:
        ...
        break
    case 218:
        data = MissionData(description: "击沉补给舰3只", require: 3, type: MissionRequireType.BATTLE,
                processor: {
                    bean in
                    var count = 0
        ...
        break
    case 226:
        data = MissionData(description: "2-1～2-5 BOSS战胜利5次", require: 5, type: MissionRequireType.BATTLE,
                processor: {
                    bean in
                    var count = 0
                    if let battleBean = bean as? IBattleResult {
        ...
        break
    case 230:
        data = MissionData(description: "击沉潜水舰6只", require: 6, type: MissionRequireType.BATTLE,
                processor: {
                    bean in
                    var count = 0
                    count += Battle.instance.enemyList.filter { (ship: Ship) -> Bool in
    ...
    case 619:
        ...
        break
    case 673:
        data = MissionData(description: "废弃小口径主炮4个", require: 4, type: MissionRequireType.DESTROY_ITEM,
                processor: {
                    bean in
                    var count = 0
        ...
        break
    case 674:
        data = MissionData(description: "废弃机枪3个", require: 3, type: MissionRequireType.DESTROY_ITEM,
                processor: {
                    bean in
                    var count = 0
                    if let b = bean as? JsonBean {
